...
- 返り値も所有権が移動する。
- 引数の所有権を返り値で返すこともできる。
- 参照を使うことで、所有権を移動せずに値を借用することもできる。
- 参照でも、ミュータブルな参照を使うことで、所有権を移動せずに値を変更することもできる。

```rust
// &mut SqlConnection みたいな参照を引数にすることで、所有権を移動せずに値を借用することもできる。(可変)
// &string みたいな参照を引数にすることで、所有権を移動せずに値を借用することもできる。(不変)
fn sqlinsert(conn : &mut SqlConnection, query : &str) -> Result<()> {
} 
```
