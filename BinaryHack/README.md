...
### PIC(Portable Independent Code)とは

メモリ上のどこに配置しても動くということ、`gcc/g++`のオプションで付けるアレのこと。
以下の違いがあり、Linuxで`共有ライブラリ`を作成する際に使用される。

1. -fpic は、オーバーヘッドが小さく、実効速度が比較的速い。しかし、OSによってサイズの制限がある場合がある。
2. -fPIC は、サイズの制限を回避できる。
